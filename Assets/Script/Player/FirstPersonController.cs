...
public class FirstPersonController : MonoBehaviour
{
    ...
    #region water
    [Header("Liquid")]
    public float waterGravityApplySpeed;
    public float waterGravityScale;
    public float waterMoveSpeed;
    public float waterUpSpeed;

    public float lavaGravityApplySpeed;
    public float lavaGravityScale;
    ...
    private void Awake()
    {
        ...
        StartCoroutine(MakeWalkSound());
    }

    IEnumerator MakeWalkSound()
    {
        while (true)
        {
            yield return new WaitUntil(() => isWalking && isGrounded && !im.showingInventoryUI && !pm.paused && !inWater);

            sm.PlaySound("Walk", 1);
            float delay = 0.2f;
            yield return new WaitForSeconds(Input.GetKey(KeyCode.LeftControl) ? delay * 0.7f: delay);
        }
    }
    ...
    private void Update()
    {
        ...
        if (inWater && Input.GetKey(jumpKey))
        {
            JumpInWater();
        }
        else if(inLava && Input.GetKey(jumpKey))
        {
            JumpInLava();
        }
        else if(enableJump && Input.GetKeyDown(jumpKey) && isGrounded)
        {
            Jump();
        }
    ...
    void FixedUpdate()
    {
        ...
        else if (lm.loading || im.showingInventoryUI)
        {
            rb.freezeRotation = true;
            rb.velocity = new Vector3(0, rb.velocity.y, 0);

            //knock back
            UpdateVelocityEffect();
            return;
        }
        #region Movement
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown && !isCrouched && hungerM.hunger > 6)
            {
                targetVelocity = transform.TransformDirection(targetVelocity) * sprintSpeed * (inWater ? waterMoveSpeed : 1) * (inLava ? lavaMoveSpeed : 1);
            ...
            else
            {
                ...
                targetVelocity = transform.TransformDirection(targetVelocity) * walkSpeed * (inWater ? waterMoveSpeed : 1) * (inLava ? lavaMoveSpeed : 1);
    ...
    public void UpdateVelocityEffect()
    {
        //knockback
        ...
        if (inWater && rb.velocity.y < waterGravityScale)
        {
            
            rb.velocity = Vector3.Lerp(rb.velocity, new Vector3(rb.velocity.x, waterGravityScale, rb.velocity.z), waterGravityApplySpeed*Time.deltaTime);
        }

        if (inLava && rb.velocity.y < lavaGravityScale)
        {

            rb.velocity = Vector3.Lerp(rb.velocity, new Vector3(rb.velocity.x, lavaGravityScale, rb.velocity.z), lavaGravityApplySpeed * Time.deltaTime);
        }
    }
    ...
    private void CheckGround()
    {
        ...
        if (Physics.Raycast(origin, direction, out RaycastHit hit, distance, walkableLayer))
        {
            Debug.DrawRay(origin, direction * distance, Color.red);
            isGrounded = true;

            if (!preGrounded && rb.velocity.y<-2)
            {
                sm.PlaySound("Land", 1);
            }
        }
    ...
    {
        rb.velocity = new Vector3(rb.velocity.x, waterUpSpeed, rb.velocity.z);
    }
    private void JumpInLava()
    {
        rb.velocity = new Vector3(rb.velocity.x, lavaUpSpeed, rb.velocity.z);
    }
    ...
    {
        //water
        if (Physics.OverlapSphere(transform.position + Vector3.up * headHeight, headSize, waterLayer).Length != 0)
        {
            headInWater = true;
        }
        else
        ...
        if (Physics.OverlapCapsule(transform.position + Vector3.down * 0.7f, transform.position + Vector3.up * 0.7f, 0.5f, waterLayer).Length != 0)
        {
            inWater = true;
        }
        else
        ...
        //lava
        if (Physics.OverlapSphere(transform.position + Vector3.up * headHeight, headSize, lavaLayer).Length != 0)
        {
            headInLava = true;
        }
        else
        ...
        if (Physics.OverlapCapsule(transform.position + Vector3.down * 0.7f, transform.position + Vector3.up * 0.7f, 0.5f, lavaLayer).Length != 0)
        {
            inLava = true;
        }
        else
